...
Agent Plugins for AWS equip AI coding agents with the skills to help you architect, deploy, and operate on AWS. Agent plugins are currently supported by Claude Code, Cursor, and OpenCode.
...
- `amplify-workflow` - AWS Amplify Gen 2 development
- `api-gateway` - Amazon API Gateway development
- `aws-lambda` - AWS Lambda serverless development
- `aws-lambda-durable-functions` - AWS Lambda durable functions for long-running workflows
- `aws-serverless-deployment` - AWS SAM and CDK deployment
- `deploy` - Deploy applications to AWS
- `dsql` - Aurora DSQL database development
- `gcp-to-aws` - GCP to AWS migration

When working with OpenCode in this repository, simply invoke the skill tool to load the relevant skill for your task. For example:
- Use `deploy` skill when you want to deploy applications to AWS
- Use `amplify-workflow` skill when building full-stack apps with AWS Amplify
